...
| [Hermes Agent](https://github.com/NousResearch/hermes-agent) 0.18.x | The runtime Zam is built on. Third-party, MIT, by Nous Research — installed and authenticated separately; see below. |
| A Slack workspace you control | Zam talks through a Slack app you create there. |
...
Zam does not vendor it and does not install it for you. It is
...
Zam reads the board, sends Slack messages and schedules cron through the
`hermes` CLI as a subprocess — never by importing it — so the version that
matters is the one on your `PATH`. Zam is developed against **0.18.x**. Newer
...
This only creates `~/.config/cuzam/config.yaml` and a managed CLI
...
1. Go to <https://api.slack.com/apps> → **Create New App** → **From an app
   manifest** and paste `slack/cuzam-slack-manifest.json` (readable guide:
...
This adds Cuzam's skills, scripts, and the isolated worker profile to an
...
- Wake management is your choice: Cuzam never changes `pmset`. If the
  machine sleeps, Zam sleeps with it.
