...
| `local-brain` | Local Brain | implemented | `qwen3.6:35b-mlx` orchestrates chat, briefs and tool decisions locally. Coding runs on Claude. | NOT writing production code; NOT cloud LLM for orchestration |
...
| `event-spine` | Event Spine | in-progress | Pipeline events in Ristretto's own log; `preflight` proves a repo is loop-capable; `gc` reclaims finished worktrees. | NOT writing into Hermes' schema; NOT owning a UI; NOT removing worktrees with uncommitted work |
