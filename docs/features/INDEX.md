...
| `autonomous-coding` | Autonomous Coding | in-progress | Zam runs a selected coding flow on a branch and opens a PR. | NOT auto-merging; NOT acting on employer systems |
| `custom-model-flows` | Custom Model Flows | in-progress | Named flows (`full`, `short`) trade scrutiny for speed across plan, build, review, verify, PR. Providers declare hosting. | NOT arbitrary commands; NOT storing credentials; NOT inferring hosting from a URL |
