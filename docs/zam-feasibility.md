# Zam — feasibility check

> Superseded by `docs/zam-architecture.md` for scope and purpose. This note
...
Zam is the working name for the embodied version of Cuzam: an
...
This matters more for Zam than for a chat bot. A bot that misses a message
...
`cuzam/doorbell.py` defines a closed `MILESTONES` set, and the comment on
...
not. Zam v1 does not need screen awareness — it needs to surface exactly the
...
### Zam is a client, not a fourth system

The event spine, approvals, fleet data and `/chat` already exist and are
running. Zam is another surface on the same core, the way the dashboard is.
Framing it as four independent layers risks rebuilding `cuzam/dash/`.
...
**The rule:** voice composes, a click commits. Zam transcribes, writes back
...
- **Zam does not speak.** You talk, Zam writes, in a conversation globe.
- **No TTS.**

```
button/hotkey → mic → whisper (local) → text
    → Zam (the existing /chat endpoint)
...
hosting a web view of a Zam page served by the existing dashboard server.
...
Zam v1 makes the dashboard's read-only limit the binding constraint. "I
talk, he writes" is a status reader unless Zam can act on what was said, and
acting means launching work. The launch surface is therefore a prerequisite
for Zam being more than a nicer way to read the fleet view — not a competing
priority.
...
`zam/Zam.swift`, built by `scripts/build-zam.sh` into `Zam.app`.
...
**Hold the mouse on Zam to talk.** The microphone opens on mouse-down and
...
polled from `/zam/state` every 20 seconds.

Zam has no tools and no board access. It posts to two endpoints and renders
...
- The dashboard address is read from `~/.cuzam/dash-url`, written by the
