...
> the go. The dashboard as a masterboard. Zam floating on my computer, ready
> to help when needed.

And the line that settles the architecture:

> I can just start a project without Zam if I'm in my computer.

Zam is not how work gets executed — a terminal already does that better. Zam
...
```
   ...
   you review     read it on a phone, merge it
      │
      ▼
   "deploy it"    Zam runs the deploy loop and watches production
      │
      ▼
...
What Zam needs is organised around **your work**, which lives in a work
...
A run is something Zam *does about* a piece of work, the way sending a Slack
...
face, the masterboard and Slack must show the same Zam with the same memory.
Today they are three separate entry points to a stateless proxy, which is why
Zam cannot answer "the thing I mentioned yesterday".
...
**Zam — making it happen when nobody is watching.** Dispatch, model routing
across stages, process supervision, artifacts, events, memory, and the
surfaces you reach it through.

Zam is not a UI layer over wayworks. A UI implies you are present, and the
...
So the correction is not "make Zam a UI". It is: **the stage definitions move
to wayworks and Zam executes them** — exactly the relationship the two
already have around `.cc-verify`, which wayworks defines and Zam only reads.

### The seam is undefended

Zam depends on wayworks' conventions by reading files from disk. If
...
**Decided: Zam owns its loop.** Conversation state, a memory it controls, an
...
surfaces requires owning session state; Zam needs a specific tool set rather
...
- **Zam in conversation** gets search: grep the summaries, read the two or
  three that match.
...
load-bearing, and is why Zam must write careful ones.
...
`07-Archive`. Zam is a well-behaved vault citizen, not a new database.

This is a real constraint, not a detail. It means memory is *inspectable* and
survives Zam being rewritten, and it means Zam's recall problem is a search
problem over notes rather than a context-window problem.

### Acting

Zam proposes; you commit. The approval store already implements exactly
...
`docs/zam-feasibility.md` and it holds here.

## Deploy: built, never usable

The loop you actually want ends at a working deployment, not a pull request:
merge it, tell Zam, and hear either "here is your live fix" or "we need to
fix this".
...
Zam is a repository you download and run on your own Mac. That is a product
...
Zam to remember anything, and a database would trade an inspectable,
portable memory for an opaque one.

**Linear, deliberately, with a shallow seam.** Zam needs to know what is on
...
The seam is kept shallow instead. Zam reaches the work source through a
...
so a stranger must install a third-party agent runtime before Zam runs at
...
**A stranger's vault has no `_agent/` rules.** Decided: Zam brings its own
convention and scaffolds it on first run — *when the vault does not already
have one*. Where a vault carries agent rules already, Zam obeys them.
...
`make setup` should take someone from a cloned repository to a working Zam:
...
**Prompt injection gets sharper.** Zam will read issue text, vault notes and
...
keys and persona all say Cuzam. Renaming touches everything and collides
...
Zam its own agent loop with conversation state, so you can answer the brief —
