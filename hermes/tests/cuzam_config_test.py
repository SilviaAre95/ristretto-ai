...
from cuzam import events, gc, preflight
from cuzam.cli import main as cli_main
from cuzam.config import (
    ConfigError,
    doctor,
    entry_differences,
    merge_layers,
...
class ConfigTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.config, _ = load_config(ROOT / "cuzam.yaml")
    ...
    def test_environment_model_override(self) -> None:
        flow = resolved_flow(
            ...
            "local-custom",
            {"ZAM_LOCAL_BRAIN_MODEL": "local-test-model"},
        )
    ...
    def test_instance_environment_overrides_direct_value(self) -> None:
        ...
        self.assertEqual(
            instance_value(config, "linear_team", {"CUZAM_LINEAR_TEAM": "ENV"}),
            "ENV",
        )
    ...
    def test_configure_writes_valid_user_settings(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            target.write_text((ROOT / "cuzam.yaml").read_text(encoding="utf-8"))
            result = cli_main(
                [
                    "--config",
                    str(target),
...
class PrStageTests(unittest.TestCase):
    ...
    def test_run_artifacts_do_not_count_as_uncommitted(self) -> None:
        with contextlib.ExitStack() as stack:
            cwd = self._repo(stack)
            (cwd / ".cuzam" / "runs").mkdir(parents=True)
            (cwd / ".cuzam" / "runs" / "plan.md").write_text("plan\n", encoding="utf-8")
...
class ConfigLayerTests(unittest.TestCase):
    """The user's file describes a machine, not how Cuzam works.
    ...
    def setUp(self) -> None:
        ...
        self.project, _ = load_config(ROOT / "cuzam.yaml")
...
class GarbageCollectionTests(unittest.TestCase):
    ...
    def test_run_artifacts_do_not_block_removal(self) -> None:
        path = self._worktree("t_a1b2c3d4")
        (path / ".cuzam" / "runs").mkdir(parents=True)
        (path / ".cuzam" / "runs" / "plan.md").write_text("plan\n", encoding="utf-8")
        decision = self._only({"t_a1b2c3d4": {"id": "t_a1b2c3d4", "status": "done"}})
        self.assertEqual(decision.action, "remove")
