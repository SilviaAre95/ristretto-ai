...
try:
    ...
    from cuzam.dash.app import app
...
class BindTests(unittest.TestCase):
    ...
    def test_tailnet_is_preferred_when_available(self) -> None:
        with mock.patch("cuzam.dash.serve.tailnet_address", return_value="100.64.0.1"):
            self.assertEqual(resolve_host(), ("100.64.0.1", "tailnet"))

    def test_falls_back_to_loopback_not_all_interfaces(self) -> None:
        with mock.patch("cuzam.dash.serve.tailnet_address", return_value=None):
            host, _ = resolve_host()
    ...
    def test_the_post_surface_is_exactly_what_we_intend(self) -> None:
        ...
        self.assertEqual(
            posting,
            {
                ...
                "/chat",
                "/launch",
                # Zam's ear. It changes nothing and spends no tokens, but it
...
class CrossSiteTests(unittest.TestCase):
    ...
    def test_chat_is_same_origin_too(self) -> None:
        ...
        from cuzam.assistant import loop
...
class ControlActionTests(unittest.TestCase):
    ...
    def test_stop_reports_failure_verbatim(self) -> None:
        # zam-stop.sh exits non-zero with NOT STOPPED when the kill did not
...
class LinkAndCopyTests(unittest.TestCase):
    ...
    def test_a_denial_is_spelled_denied(self) -> None:
        ...
        source = (
            Path(__file__).resolve().parents[2] / "cuzam" / "dash" / "app.py"
        ).read_text()
...
class LaunchCoreTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # layer (~/.config/cuzam/config.yaml), which CI does not have, so
...
class VoiceTests(unittest.TestCase):
    """Transcription. Zam must never die on a bad clip."""

    def test_nothing_recorded_is_not_a_crash(self) -> None:
        from cuzam import voice
    ...
    def test_the_model_is_overridable(self) -> None:
        from cuzam import voice
        self.assertEqual(
            voice.model_name({"CUZAM_WHISPER_MODEL": "mlx-community/whisper-large-v3-mlx"}),
            "mlx-community/whisper-large-v3-mlx",
        )
        self.assertEqual(voice.model_name({}), voice.DEFAULT_MODEL)

    def test_a_missing_engine_says_so(self) -> None:
        # The dashboard runs without mlx-whisper; only Zam needs it.
        from cuzam import voice
...
class ZamStateTests(unittest.TestCase):
    """The glance Zam takes on a timer, forever."""
    ...
    def test_it_reports_what_is_waiting(self) -> None:
        with mock.patch.object(approvals, "pending", return_value=[{"id": "a"}, {"id": "b"}]):
            body = self.client.get("/zam/state").json()
    ...
    def test_an_unreadable_store_does_not_break_the_face(self) -> None:
        with mock.patch.object(approvals, "pending", side_effect=RuntimeError("gone")):
            response = self.client.get("/zam/state")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["waiting"], 0)


class VocabularyTests(unittest.TestCase):
    """The decoding hint. Zam's own name came back as "Neymar"."""

    def test_it_knows_its_own_name(self) -> None:
        from cuzam import voice
        self.assertIn("Zam", voice.prompt())
    ...
    def test_no_private_names_are_written_down_here(self) -> None:
        ...
        from cuzam import voice
        source = (Path(__file__).resolve().parents[2] / "cuzam" / "voice.py").read_text()
    ...
    def test_an_unreadable_config_still_produces_a_hint(self) -> None:
        from cuzam import voice
        with mock.patch("cuzam.config.load_config", side_effect=RuntimeError("gone")):
            self.assertIn("Zam", voice.prompt())
...
class ReloadTests(unittest.TestCase):
    ...
    def test_reload_watches_the_package_and_nothing_else(self) -> None:
        ...
        self.assertTrue(watched[0].endswith("cuzam"))
