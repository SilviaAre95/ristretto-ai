...
from cuzam import approvals, broker, events, runner
...
class BrokerTest(ApprovalStoreTest):
    ...
    def setUp(self) -> None:
        ...
        patcher.start()
        self.addCleanup(patcher.stop)
        self.env = {"CUZAM_TASK_ID": "t_a1b2c3d4", "CUZAM_APPROVAL_TIMEOUT": "60"}
    ...
    def test_no_owning_task_is_a_deny(self) -> None:
        ...
        self.assertIn("CUZAM_TASK_ID", result["message"])
...
class BrokerWiringTest(unittest.TestCase):
    ...
    def test_the_broker_runs_on_an_interpreter_that_can_import_it(self) -> None:
        ...
        self.assertEqual(config["args"], ["-m", "cuzam.broker"])
