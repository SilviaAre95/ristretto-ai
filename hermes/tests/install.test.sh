...
tmp="$(mktemp -d "${TMPDIR:-/tmp}/cuzam-install-test.XXXXXX")"
trap 'rm -rf "$tmp"' EXIT

config_dir="$tmp/config"
bin_dir="$tmp/bin"
export CUZAM_CONFIG_DIR="$config_dir"
export CUZAM_BIN_DIR="$bin_dir"
export CUZAM_SKIP_SETUP=1
...
assert "installer creates a CLI symlink" test -L "$bin_dir/cuzam"
assert "installer creates user configuration" test -f "$config_dir/config.yaml"
assert "installed configuration validates" "$bin_dir/cuzam" --config "$config_dir/config.yaml" validate
...
assert "default uninstall removes only the managed CLI link" test ! -e "$bin_dir/cuzam"
...
if [ "${1:-}" = "profile" ] && [ "${2:-}" = "create" ]; then
  mkdir -p "$HERMES_HOME/profiles/zam-worker/skills"
  exit 0
fi
...
case " $* " in
  *" hooks list "*)
    cat "$HERMES_HOME/profiles/zam-worker/config.yaml" 2>/dev/null
    exit 0
    ;;
esac
exit 0
EOF
chmod +x "$fakebin/cuzam" "$fakebin/hermes"

PATH="$fakebin:$PATH" CUZAM_HERMES_HOME="$fake_home" \
...
assert "reinstall keeps exactly one flow-guard block" \
  test "$(grep -c 'zam:flow-guard begin' "$fake_home/profiles/zam-worker/config.yaml")" -eq 1
assert "flow guard is declared as a list, not a string" \
  grep -q '^    - matcher: "kanban_complete"' "$fake_home/profiles/zam-worker/config.yaml"
...
# plugin registers no commands, so !zam-approve silently does nothing — which
# is exactly how it shipped the first time.
assert "installer links the approvals plugin" \
  test -e "$fake_home/plugins/zam-approvals/plugin.yaml"
assert "installer enables the approvals plugin" \
  grep -q "plugins enable zam-approvals" "$repo/scripts/install-hermes.sh"
...
assert "doorbell can find tailscale on PATH" \
  grep -q "/usr/local/bin" "$repo/hermes/scripts/zam-doorbell.sh"
