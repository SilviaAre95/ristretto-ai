...
SPEC = importlib.util.spec_from_file_location(
    "zam_launch_plugin", ROOT / "hermes" / "plugins" / "zam-launch" / "__init__.py"
)
...
class ParseTest(unittest.TestCase):
    def setUp(self) -> None:
        which = mock.patch.object(plugin.shutil, "which", return_value="/usr/bin/cuzam")
    ...
    def test_no_flow_defers_to_the_configured_default(self) -> None:
        ...
        # keeps the CLI, the form and cuzam.yaml from disagreeing.
...
class SafetyTest(unittest.TestCase):
    ...
    def test_a_launcher_that_hangs_does_not_raise(self) -> None:
        with mock.patch.object(plugin.shutil, "which", return_value="/usr/bin/cuzam"), \
             mock.patch.object(subprocess, "run", side_effect=subprocess.TimeoutExpired("cuzam", 120)):
    ...
    def test_no_model_sits_in_the_launch_path(self) -> None:
        ...
        source = (ROOT / "hermes" / "plugins" / "zam-launch" / "__init__.py").read_text()
        import re
