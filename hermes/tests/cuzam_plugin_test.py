...
SPEC = importlib.util.spec_from_file_location(
    "zam_approvals_plugin", ROOT / "hermes" / "plugins" / "zam-approvals" / "__init__.py"
)
...
class CommandTest(unittest.TestCase):
    def setUp(self) -> None:
        which = mock.patch.object(plugin.shutil, "which", return_value="/usr/local/bin/cuzam")
        which.start()
    ...
    def test_a_timeout_does_not_raise(self) -> None:
        self.run.side_effect = subprocess.TimeoutExpired("cuzam", 30)
...
class RegistrationTest(unittest.TestCase):
    def test_it_registers_the_three_commands(self) -> None:
        ...
        plugin.register(Ctx())
        self.assertEqual(
            sorted(registered), ["zam-approve", "zam-deny", "zam-pending"]
        )
    ...
    def test_no_model_sits_in_the_decision_path(self) -> None:
        ...
        source = (ROOT / "hermes" / "plugins" / "zam-approvals" / "__init__.py").read_text()
...
class ArgumentParsingTest(CommandTest):
    ...
    def test_client_boilerplate_is_not_taken_for_an_id(self) -> None:
        # Slack delivered "!zam-approve *Sent using* Claude ...", so "*Sent"
