...
from cuzam import doorbell, events
...
class ApprovalNotificationTest(unittest.TestCase):
    ...
    def test_it_says_how_to_answer_and_says_DM(self) -> None:
        ...
        text = doorbell.compose(self.event(id="a1b2"), "http://zam:8787")
        self.assertIn("DM me", text)
        self.assertIn("!zam-approve a1b2", text)
        self.assertIn("!zam-deny a1b2", text)

    def test_it_still_links_to_the_dashboard(self) -> None:
        text = doorbell.compose(self.event(id="a1b2"), "http://zam:8787")
        self.assertIn("http://zam:8787/task/t_a1b2c3d4", text)

    def test_a_missing_id_does_not_produce_a_dangling_command(self) -> None:
        text = doorbell.compose(self.event(), "http://zam:8787")
        self.assertNotIn("!zam-approve  ", text)
