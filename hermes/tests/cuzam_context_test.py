...
from cuzam import context, runner  # noqa: E402
...
class IgnoreArtifactsTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        (self.repo / ".cuzam" / "runs" / "t_x").mkdir(parents=True)
        (self.repo / ".cuzam" / "runs" / "t_x" / "context.md").write_text(
...
class SecretLoadingTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        (self.home / ".config" / "cuzam").mkdir(parents=True)
        self.hermes = self.home / ".hermes" / ".env"
        self.own = self.home / ".config" / "cuzam" / "env"
    ...
    def test_cuzam_own_file_wins_over_hermes(self) -> None:
        self.hermes.write_text("LINEAR_API_KEY=hermes\n", encoding="utf-8")
        self.own.write_text("LINEAR_API_KEY=cuzam\n", encoding="utf-8")
        target = self.env()

        with mock.patch.object(self.cfg, "secret_names", return_value={"LINEAR_API_KEY"}):
            self.cfg.load_env(target)

        self.assertEqual(target["LINEAR_API_KEY"], "cuzam")
    ...
    def test_declared_names_come_from_the_config(self) -> None:
        names = self.cfg.secret_names({
            "instance": {"linear_team_env": "ZAM_TEAM", "name": "Zam"},
            "providers": {"p": {"auth_token_env": "P_TOKEN", "runner": "claude-code"}},
        })

        self.assertIn("ZAM_TEAM", names)
