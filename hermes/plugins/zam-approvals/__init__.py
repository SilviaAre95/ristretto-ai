"""Answer a blocked Cuzam flow from Slack.
...
`!zam-approve` rather than `/zam-approve`: Slack refuses native slash commands
...
# and people write "!zam-approve please". Treating that as an id produced
...
def _run(args: list[str]) -> tuple[int, str]:
    binary = _cuzam()
    if not binary:
        return 1, "cuzam is not on PATH for the gateway process."
    try:
...
def _decide(verdict: str, raw_args: str) -> str:
    ...
    # "not" in "!zam-deny not right now" is perfectly id-shaped, and chat
...
def register(ctx) -> None:
    ctx.register_command(
        "zam-pending",
        handler=pending,
        description="List Cuzam flows waiting on an approval.",
    )
    ctx.register_command(
        "zam-approve",
        handler=approve,
        description="Allow what a waiting Cuzam flow asked to do.",
        args_hint="[request-id]",
    )
    ctx.register_command(
        "zam-deny",
        handler=deny,
        description="Refuse what a waiting Cuzam flow asked to do.",
