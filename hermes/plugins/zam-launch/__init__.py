...
`!zam-start ABC-42` — default flow, attended. `!zam-start ABC-42 short
...
# A launch shells `cuzam launch`, which creates a board task and dispatches
...
# Only the flows Zam actually defines. Anything else is a typo, and the CLI
...
def start(raw_args: str = "") -> str:
    ...
    binary = _cuzam()
    if not binary:
        return "cuzam is not on PATH for the gateway process."

    tokens = (raw_args or "").split()
    if not tokens:
        return "Which issue? e.g. !zam-start ABC-42 short unattended"
    ...
    if project is None:
        return (
            ...
            f"Add it: !zam-start {issue}{' ' + flow if flow else ''} project:<name>"
        )
...
def register(ctx) -> None:
    ctx.register_command(
        "zam-start",
        handler=start,
        description="Start a supervised run: !zam-start <issue> [flow] [unattended] project:<name>",
