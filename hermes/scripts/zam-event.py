#!/usr/bin/env python3
"""Emit one Cuzam pipeline event from a shell call site.
...
either.
    ...
    zam-event.py <task_id> <kind> [--issue KEY] [--project NAME]
...
try:
    ...
    print(f"zam-event: unavailable: {exc}", file=sys.stderr)
    raise SystemExit(0)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="zam-event")
    ...
    if args.payload:
        try:
            ...
            print(f"zam-event: ignoring unparsable payload: {exc}", file=sys.stderr)
    ...
    try:
        ...
        print(f"zam-event: {exc}", file=sys.stderr)
    return 0
