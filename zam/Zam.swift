// Zam — Cuzam's face on the desktop.
//
...
// and it writes back what it heard and what Zam said.
//
// Three deliberate choices:
//
// Non-activating panel. Clicking Zam must not pull focus out of whatever you
...
// detector.
//
// It only ever asks Zam. Zam has no tools and no board access of its own; it
...
struct Config {
    // The dashboard this Zam belongs to. Defaults to the tailnet address the
    // service binds; ZAM_DASH overrides for a laptop or a test instance.
    static var dashURL: String {
        if let explicit = ProcessInfo.processInfo.environment["ZAM_DASH"], !explicit.isEmpty {
            return explicit
        }
        ...
        let file = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".cuzam/dash-url")
...
/// from Zam is a sentence, not an exception. This says exactly that.
enum Answer {
    case said(String)
    case failed(String)
}
...
final class Ear {
    ...
    /// True if the microphone actually opened. Zam says so rather than
    ...
    func start() -> Bool {
        ...
        let target = FileManager.default.temporaryDirectory
            .appendingPathComponent("zam-\(UUID().uuidString).wav")
        do {
...
final class Zam: NSObject, NSApplicationDelegate {
...
let zam = Zam()
app.delegate = zam
// .accessory: no Dock icon, no menu bar. Zam is furniture, not an app you
// switch to.
