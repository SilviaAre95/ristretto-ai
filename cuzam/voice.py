...
Zam listens only while you hold the key down, so this is handed a short
...
# is the cheap fix for proper nouns: the first thing Zam ever heard was
...
VOCABULARY = (
    "Zam", "Zam", "Cuzam", "Hermes", "Linear", "Slack",
...
def transcribe(audio: bytes, suffix: str = ".wav", environ: Mapping[str, str] | None = None) -> Heard:
    """Words from a recording. Never raises — Zam must not die on a bad clip."""
    if not audio:
    ...
    except Exception as exc:  # noqa: BLE001 - a bad clip must not take Zam down
...
def warm(environ: Mapping[str, str] | None = None) -> bool:
    ...
    when Zam starts rather than when you first speak is the difference
