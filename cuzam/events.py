"""Cuzam's own pipeline event log.
...
are Cuzam's vocabulary, so Cuzam keeps its own append-only log rather
...
def emit(
    ...
    *,
    ...
    try:
        ...
        print(f"zam-event: payload for {kind} is not serializable: {exc}", file=sys.stderr)
        body = None
    try:
        ...
        print(f"zam-event: could not record {kind}: {exc}", file=sys.stderr)
        return False
    return True
...
def read(
    ...
    try:
        ...
        print(f"zam-event: could not read events: {exc}", file=sys.stderr)
        return []
    result = []
    for row in rows:
        item = dict(row)
