...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(prog="cuzam")
    ...
    chat_command = commands.add_parser(
        "chat", help="ask Zam a question (its agent loop, with tools and memory)"
    )
    ...
    try:
        ...
        if args.command == "dash":
            try:
                ...
                print(
                    f"cuzam: the dashboard needs its extra dependencies: {exc}\n"
                    "  pip install 'cuzam-ops[dash]'",
                    file=sys.stderr,
                )
                return 2
            try:
                ...
                print(f"cuzam: {exc}", file=sys.stderr)
                return 2
