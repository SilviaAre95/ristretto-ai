"""Assemble the fleet view from the kanban board and Cuzam's event log.

Hermes owns task lifecycle; Cuzam owns what happened inside a run. Neither
...
instead from the age of the newest signal we do have — a Cuzam event, or
...
def running_flows(timeout: int = 10) -> set[str]:
    ...
    # so any shell mentioning "cuzam.runner --task-id" — including a
    ...
    for line in listing.stdout.splitlines():
        if "-m cuzam.runner" not in line or "--task-id" not in line:
            continue
