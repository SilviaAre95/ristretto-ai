...
def run(host: str | None = None, port: int = 8787, reload: bool = False) -> int:
    ...
    print(f"zam-dash: http://{bind}:{port}  ({why}){'  [reloading]' if reload else ''}")
    if bind == LOOPBACK and not host:
        ...
        print(
            "zam-dash: WARNING bound to loopback — every link in every "
            "notification points at the tailnet address and will not "
            "resolve. Restart once Tailscale is up: "
            "launchctl kickstart -k gui/$(id -u)/com.cuzam.dash",
            file=sys.stderr,
        )
    uvicorn.run(
        "cuzam.dash.app:app",
