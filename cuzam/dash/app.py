"""Fleet view over the kanban board and Cuzam's event log.
...
app = FastAPI(title="Zam", docs_url=None, redoc_url=None, openapi_url=None)
...
@app.post("/chat")
async def ask_zam(request: Request) -> JSONResponse:
    """Put a question to Zam with the fleet as context.
    ...
    # Zam's own loop now, not the Hermes proxy: it calls Zam's tools and
...
async def hear(request: Request) -> JSONResponse:
    """Words from a recording Zam made while you held the key.
